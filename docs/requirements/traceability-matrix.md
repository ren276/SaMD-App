...
| REQ-ABH-02 | `presentation/abha/AbhaLoginScreen`+`AbhaOtpScreen`, `domain/usecase/VerifyAbhaLoginUseCase`, `RegisterViewModel.loadAbhaProfile`, `abha-field-mapping.md`. Masked-mobile guard (Phase 6c W2, 2026-08-18): `AbhaProfile.kt#isMaskedAbhaMobile`, `RegisterViewModel.canSubmit` | H-06 | Manual (nav flow reviewed) | ✓ VerifyAbhaLoginUseCaseTest, RegisterViewModelTest (autofill, masked-mobile-does-not-satisfy-REQ-REG-01, manual-entry-does) |
...
| — (Phase 6c Part 2 infra, 2026-08-18, no new REQ-ABH row) | `domain/abha/AbdmAbhaSource`, `data/remote/RetrofitAbhaSource`, `data/remote/api/AbhaApiService`, `data/remote/dto/AbhaDto.kt`, DI wiring in `di/NetworkModule.kt`. `Patient`/`PatientEntity` provenance columns (`MIGRATION_13_14`, Part 1): `abhaAddress`, `kycVerified`, `verificationSource`, `verifiedAt` | H-03 (no auto-link, verified but unused by the actual use cases yet) | `MigrationTest13To14` on device (2/2), `compileDevDebugKotlin` | Not yet exercised by `CreateAbhaProfileUseCase`/`VerifyAbhaLoginUseCase` (deliberately deferred, see PROGRESS.md's Phase 6c Part 2 entry) |
