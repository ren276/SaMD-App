...
| REQ-SEC-06 | `app/build.gradle.kts` flavor-scoped `KERNEL_BASE_URL`/`BACKEND_BASE_URL`/`ABHA_BACKEND_BASE_URL` (staging/prod = `https://...`, dev = `http://` LAN IP) | — | Manual ✓ (build.gradle review — staging/prod URLs are `https://`) | TODO |
...
| REQ-SYN-02 | `CaseRecordRepository.assignDoctor(isOnline)`/`sendAllPendingCases`/`observePendingSyncCount` (case-record leg only); rest per `docs/sync-design.md` | H-05 | Manual ✓ | ✓ MockSyncStatusTest — PARTIAL: generic per-entity syncState is schema-only (`MIGRATION_12_13`, nothing reads/writes it yet), no WorkManager/real backend, no conflict resolution, no purge-on-sync |
