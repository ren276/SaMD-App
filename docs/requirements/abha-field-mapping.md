...
| `mobileNumber`      | `Patient.mobileNumber`            | Satisfies the ≥1 contact-method rule (REQ-REG-01) **only when the value is a full, usable number** (today, only ever true for the mock). **2026-08-17, real backend adapter (D4, `docs/requirements/abha-internal-contract.md`), historical:** ABDM's real `profile/account` response was believed to never return a full mobile number, only a masked one (e.g. `"******0903"`), based on the one recorded Postman example at the time. **Superseded, 2026-08-28 (D4, same doc, corrected):** this masked-only belief is not settled. A prior watched live run observed a full, unmasked 10-digit mobile on `profile/account.mobile`; a 2026-08-28 watched run could not adjudicate either way, since its own observation of the field was redacted before the masked/unmasked shape could be recorded. **Current status: unresolved, neither shape confirmed.** Both the original belief and `_extract_masked_mobile`'s masked-value assumption (`abdm_adapter/service.py`) are suspect pending a deliberate, unredacted recheck; the real `AbhaIdentity.mobile_number` the backend returns may carry a masked value, a full value, or `null` — never a fabricated one. The Android-side handling below already covers both observed shapes: a masked value is display-only, a full value is written into `MOBILE_NUMBER` and used, so no app-side change is required while this stays unresolved. **Fixed, 2026-08-18 (Phase 6c, W2):** `RegisterViewModel.loadAbhaProfile` detects a masked mobile (`AbhaProfile.kt`'s `isMaskedAbhaMobile` — any non-digit character in an otherwise-mobile-shaped value) and does NOT write it into the submittable `MOBILE_NUMBER` field; it surfaces as `RegisterUiState.maskedAbhaMobile` instead, a display-only hint telling the worker ABHA has a masked number on file and a real one must be entered manually. `canSubmit`'s contact-method check also independently guards against a masked-shaped value reaching `MOBILE_NUMBER` by any other path (defense in depth, not the only guard). Proven by `RegisterViewModelTest`: a masked mobile alone does not satisfy `canSubmit`; a manually-entered real number does. |
