...
| `mobile_number` | `mobile`: `"******0903"` | **Gap, not a transform.** *Historical, from the one recorded Postman example:* the real endpoint was believed to never return the full mobile number, only a masked one (last 4 digits); the pinned shape's own example (`"9876543210"`, a full number) was not achievable from this endpoint on that evidence. **Superseded, 2026-08-28:** a prior watched live run observed a full, unmasked mobile on this same field, contradicting the masked-only belief above; a 2026-08-28 run could not adjudicate either way (its own observation was redacted). **Current status: unresolved, neither shape confirmed.** Do not assume masked or unmasked; see D4 below for the full correction and the open action (a deliberate, unredacted recheck). |
...
- **D5, photo dropped, never persisted.** The real `profile/account` response carries
  `profilePhoto`/`kycPhoto` (inline base64 JPEG bytes). `abdm_adapter/mapping.py`'s
  `profile_to_abha_identity` never extracts either field: there is no `body.get("profilePhoto")`
  or `body.get("kycPhoto")` call anywhere in the function; `photo_url` is hardcoded `None`
  unconditionally, so the bytes never even reach a local variable, let alone anything the function
  returns. **Confirmed against the live wire, 2026-08-28:** both fields are present as inline
  base64, approximately 5640 bytes each; the drop behavior held against real data, not just the
  recorded example. Proven, not assumed, by
