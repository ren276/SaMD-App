...
What is logged per call: `input_sha256`, `output_sha256`, `model_version`, `endpoint`,
`http_status`, `duration_ms`, `case_token`, `worker_id`, `facility_id`, `request_id`. What is not
logged: the request body, the response body, any vital value, any complaint text. Hashes prove
which input produced which output for IEC 62304 traceability without creating a second copy of the
...
| D-7 | HMAC pseudonym instead of the raw case PK on the kernel boundary. | Phase 3 hardening. Closes the open item in the H-10 row with no contract change. |
