...
| H-06 | Unauthorised access / no real auth | Wrong actor, accountability gap | Med | High | Audit log now carries a real per-session identity (`AuthSession`/`MockAuthSession`, REQ-SEC-04) instead of the hardcoded `phc_field_worker` placeholder, so a specific sign-in maps to each audit row. **This does not reduce unauthorised-access risk** — sign-in has no credential check, anyone can type any name/role — it only improves accountability once someone is in. | **Open**: real authentication + RBAC enforcement (REQ-SEC-03) |
