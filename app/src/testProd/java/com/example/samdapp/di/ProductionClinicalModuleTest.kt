...
/**
 * Runs only under `testProdDebugUnitTest`. Proves both sides of "no mock kernel scenario and
 * no MockVitalsSource can be bound in prod" (kernel-mock production safety fix,
 ...
 *    this prod-flavor test source set — attempting one would fail to compile, since those
 *    classes are not part of the prod compilation unit at all. That absence, not a runtime
 *    flag check, is the actual proof.
 * 2. **Runtime**: `ProductionClinicalModule` (`src/prod/java/.../di/ProductionClinicalModule.kt`)
...
class ProductionClinicalModuleTest {
    ...
    @Test
    fun `prod VitalsSource binding returns an empty reading, never fabricated numbers`() = runTest {
    ...
    @Test
    fun `prod KernelFallbackSource binding always returns null, never a fabricated scenario`() = runTest {
